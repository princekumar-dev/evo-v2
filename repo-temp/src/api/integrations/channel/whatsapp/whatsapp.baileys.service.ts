...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async defineAuthState() {
    ...
    try {
      if (provider?.ENABLED) {
        this.logger.info('Using provider for auth state');
      ...
      throw new InternalServerErrorException('Failed to initialize authentication state: ' + error.message);
    }
  }
  ...
  private async createClient(number?: string): Promise<WASocket> {
    ...
    // Validate authState was properly initialized
    if (!this.instance.authState || !this.instance.authState.state) {
      const errorMsg = 'Failed to initialize authentication state - database connection may be unavailable';
