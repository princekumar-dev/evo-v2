...
export class WAMonitoringService {
  ...
  public async instanceInfo(instanceNames?: string[]): Promise<any> {
    ...
    // If specific instance names were requested, ensure they exist in the database.
    if (instanceNames && instanceNames.length > 0) {
      const dbNames = instances.map((i) => i.name);
      const inexistentInstances = instanceNames.filter((n) => !dbNames.includes(n));
